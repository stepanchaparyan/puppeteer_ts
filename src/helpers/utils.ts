...
export default class Utils {
    ...
    async compareScreenshots(screenshotPath:string, name:string, component:any = this.page) : Promise<boolean> {
        const src = 'src';
    ...
    async compareScreenshotsWithClip(screenshotPath:string, name:string, options:any) : Promise<boolean> {
        const src = 'src';
        const helpers = 'helpers';
        const screenshots = 'screenshots';
        await this.page.waitFor(1000);
    ...
    async getCorrespondingBotNumber (botName:string) : Promise<number> {
		let i;
    ...
    async deleteBot (botName:string) : Promise<boolean> {
    ...
    async clickOnCreateBotButton () : Promise<void> {
        await this.page.waitFor(500);
        await this.reload();
        const botsCount = await this.page.$$eval(BOT_SECTION.SELECTORS.ALL_BOTS, bots => bots.length);
    ...
    async createBot (botName:string) : Promise<void> {
        await this.page.waitFor(500);
		await this.page.click(NAVBAR.SELECTORS.BOTS);
